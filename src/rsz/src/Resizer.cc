...
void Resizer::initBlock()
{
  ...
  // Build spatial index for soft blockage overlap queries
  buildSoftBlockageRtree();
...
void Resizer::buildSoftBlockageRtree()
{
  soft_blockages_.clear();
  soft_blockage_rtree_.clear();
  ...
  for (odb::dbBlockage* blockage : block_->getBlockages()) {
    if (blockage->isSoft()) {
      ...
      soft_blockages_.push_back(rect);
      size_t idx = soft_blockages_.size() - 1;
      RtreeBox box(RtreePoint(rect.xMin(), rect.yMin()),
                   RtreePoint(rect.xMax(), rect.yMax()));
      values.emplace_back(box, idx);
    }
  }

  if (!values.empty()) {
    soft_blockage_rtree_ = SoftBlockageRtree(values.begin(), values.end());
  }
}
...
void Resizer::setLocation(dbInst* db_inst, const Point& pt)
{
  ...
  // Check for soft blockage proximity/overlap using R-tree spatial index
  if (!soft_blockages_.empty()) {
    dbMaster* master = db_inst->getMaster();
    int width = master->getWidth();
    int height = master->getHeight();

    // Expand query box by proximity threshold to catch nearby soft blockages
    const int proximity = 10;
    RtreeBox query_box(
        RtreePoint(x - proximity, y - proximity),
        RtreePoint(x + width + proximity, y + height + proximity));
    std::vector<RtreeValue> results;
    soft_blockage_rtree_.query(bgi::intersects(query_box),
                               std::back_inserter(results));

    if (!results.empty()) {
      // Among all candidate blockages, pick the one requiring minimum
      // displacement. Clamp the buffer's lower-left to the nearest valid
      // position inside each blockage and choose the candidate with smallest
      // Manhattan displacement.
      int best_x = x, best_y = y;
      int best_disp = std::numeric_limits<int>::max();

      for (const auto& result : results) {
        const odb::Rect& blockage_rect = soft_blockages_[result.second];

        // Skip blockages that are too small to contain this buffer
        if (blockage_rect.xMax() - blockage_rect.xMin() < width
            || blockage_rect.yMax() - blockage_rect.yMin() < height) {
          continue;
        }

        // Nearest valid lower-left inside blockage
        int nx = std::max(blockage_rect.xMin(),
                          std::min(x, blockage_rect.xMax() - width));
        int ny = std::max(blockage_rect.yMin(),
                          std::min(y, blockage_rect.yMax() - height));

        int disp = std::abs(nx - x) + std::abs(ny - y);
        if (disp < best_disp) {
          best_disp = disp;
          best_x = nx;
          best_y = ny;
      ...
      if (best_disp < std::numeric_limits<int>::max()) {
        x = best_x;
        y = best_y;
        debugPrint(logger_,
                   RSZ,
                   "soft_blockage",
                   1,
                   "Buffer {} moved into soft blockage at ({}, {})",
                   db_inst->getName(),
                   x,
                   y);
      }
    ...
    for (dbInst* inst : buffers) {
      ...
      if (!optimized) {
        ...
        for (auto iterm : inst->getITerms()) {
            ...
            else if (iterm->getIoType() == odb::dbIoType::OUTPUT) output_term =
      iterm;
        }

        if (input_term && output_term && input_term->getNet() &&
      output_term->getNet()) { dbNet* in_net = input_term->getNet(); dbNet*
      out_net = output_term->getNet();
